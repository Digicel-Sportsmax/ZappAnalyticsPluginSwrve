...
/**
 The player view object.
 */
@property (nonatomic, strong) APPlayerView *playerView;


/**
 Returns A <code>APPlayer</code> object initialized with the movie at the specified URL.
 @param urlAsset The asset with the location of the movie file. This file must be located either in your application directory or on a remote server
 */
- (id)initWithContentUrlAsset:(AVURLAsset *)urlAsset;
 ...
 */
@property(nonatomic, copy) AVURLAsset *contentUrlAsset;
...
/**
 return true if the current player not allocalted
 */
- (BOOL) isPlayerNil;
/**
 Player controls.
...
/**
 ...
 A custom view that displays the player controls.
 This makes it possible to display the controls on a custom view and not necessarily on the player itself.
 If not set, uses a default - a view of the player.
 */
@property (nonatomic, weak) UIView *containerControlsView;
...
/**
 This property reflects the muted state of the AVPlayer instances held by the APPlayer
 This will also make sure new instances of AVPlayer are created with the same setting
 */
@property (nonatomic, assign) BOOL muted;

/**
 Set this property to YES to mute player until single tapped for the first time
 */
@property (nonatomic, assign) BOOL mutedUntilTouched;

/**
 Get playerView layer for PictureInPicture initialization.
 */
- (AVPlayerLayer*) playerViewLayer;
