...
/**< Player failed fatally */

typedef NS_ENUM(NSUInteger, APPlayerControllerPhase) {
    ...
    APPlayerControllerPhaseFailure
};
 ...
 */
- (id)initWithPlayableItem:(id<ZPPurchasableItemProtocol>)playable;

/**
 Creates a player controller instance.
 @param playableItems - An array of items that conform to <code>ZPPlayable</code>.
 @returns An initialized player instance.
 */
...
@property (nonatomic, strong, readonly) id<ZPPlayable> currentItem;   /**< The current item being processed. */
...
@property (nonatomic, assign) BOOL shouldContinuePlayingAfterAdFinishedPlaying;
@property (nonatomic, assign) BOOL alwaysResumeLastPlayingTime;

/**
 indicates whether or not playing ads
 */
@property (nonatomic, assign) BOOL isPlayingAds;
...
/**
 ...
 This controls will be used when player is displayed in Inline mode.
 */
@property (nonatomic, strong) UIView<APPlayerControls> *inlineControls;
 ...
 */
+ (void)setDefaultPlayerControlsInstance:(UIView<APPlayerControls> *)controls;

/**
 * Default Inline player controls.
 * Each player instance will use these controls by default for inline playback
 *  @return Returns setted default player controls instance. If wasn't set before returns standard applicaster's player default controls.
 */
+ (UIView<APPlayerControls> *)defaultInlinePlayerControls;
...
/**
 Ends playback and dismissing the view controller presenting the player
 @param shouldDismiss Indicating if needed to dismiss or just stop playing.
 */
- (void)stop:(BOOL)shouldDismiss;
...
/**
 sets a new current item for the player
 */
- (void)setPlayerCurrentPlayableItem:(id<ZPPlayable>)currentItem;

@property (nonatomic, assign, readonly) BOOL isRecording;

/**
remove currently presented overlay
 */
- (void)interruptionResumeTapped:(id)sender;

#pragma mark - Chromecast
- (void)playChromecast;
- (void)openChromecastPlayer;
@end
