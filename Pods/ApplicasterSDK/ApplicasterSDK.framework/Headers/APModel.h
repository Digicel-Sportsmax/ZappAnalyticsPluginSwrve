 ...
 */
@interface APModel : NSObject <APStateMachineDelegate, ZPModelProtocol>
{
	@protected
	id _object;
}
...
/**
 Returns the promo video URL if available
 */
@property (weak, readonly) NSURL *promoVideoURL;
...
/**
 Called when loading the model has completed.
 @param completion The completion block originall passed in to the `loadWithCompletionHandler:` methods.
 */
- (void)onModelLoaded:(void (^)(BOOL success, APModel *model))completion;
...
/**
 This method returns dictionary with all analytics params that relevant for the model.
 */
- (NSDictionary *)analyticsParams;

/**
This method returns nsstring with trigger of the model.
+  */
- (NSString *)analyticsTrigger;

/**
 This method returns nsstring with screenViewTitle of the model.
 */
- (NSString*)screenViewTitle;
- (NSString*)screenViewTitle:(BOOL)shouldAddDescription;
@end
