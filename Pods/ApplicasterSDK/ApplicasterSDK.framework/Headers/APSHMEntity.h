...
@interface APSHMEntity : NSObject
 ...
 */
-(void) stepToLinkRel:(NSString *)linkRel withCompletion:(void (^)(NSError *error, APSHMEntity *entity))block;

/*
 Get a specific siren action from the entity. If no action with that particular name is present return nil.
 */
-(APSHMAction *) getSirenAction:(NSString *)name;
