 ...
 */
@interface APCategory : APPurchasableItem <APFavoritesProtocol, APUICustomizationExtendedProtocol, APShareable, APContainerProtocol, ZPCategoryProtocol> {
	NSArray *_children;
...
/**
 Screen Type, can be used to define generic screens
 */
...
/**
 Checks if the category should show storefront
 @discussion
 This function returns NO if category is free or owned.
 In case both of them are not true - we check if an item level voucher exists
 @return YES if we should display storefront before taking action
 */
